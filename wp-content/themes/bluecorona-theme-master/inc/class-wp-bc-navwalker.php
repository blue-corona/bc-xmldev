	...
	class Bluecorona_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && $args->depth > 1 ) {
				...
				$atts['class']         = 'dropdown-toggle nav-link mr-0 mr-lg-4';
				$atts['id']            = 'menu-item-dropdown-' . $item->ID;
			} else {
				$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
				// For items in dropdowns use .dropdown-item instead of .nav-link.
				if ( $depth > 0 ) {
					$atts['class'] = 'dropdown-item mr-4 px-0 px-lg-2';
				} else {
			...
			$icon_html = '';
			if ( isset( $args->has_children ) && $args->has_children && $args->depth > 1 ) {
				$icon_html = '&nbsp;<span class="float-right float-lg-none pr-3 pr-lg-0"><i class="fa fa-angle-down bc_menu_icon_color" aria-hidden="true"></i><span> ';
			} else {
